...
import { Search, ShoppingCart, User, MapPin, ChevronDown, HelpCircle, Menu, PhoneCall } from "lucide-react";
    ...
    <header className="w-full bg-white shadow-sm sticky top-0 z-40">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <div className="bg-mm-red text-white font-extrabold text-2xl tracking-tighter px-3 py-1.5 flex items-center rounded-sm transition-transform duration-200 active:scale-95 shadow-md">
                <span>Media</span>
                <span className="text-mm-dark bg-white ml-0.5 px-1 py-0.2 rounded-xs select-none">Markt</span>
                {/* Logo Swirl Icon */}
          ...
          <div className="flex items-center gap-5 sm:gap-6">
            ...
            <div className="hidden lg:flex flex-col text-left">
              <span className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">Tu tienda</span>
      ...
      <div className="p-3 bg-gray-50 border-b border-gray-100 block md:hidden">
        <div className="relative">
          ...
          />
          <Search className="absolute right-3.5 top-2.5 text-gray-400" size={16} />
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-gray-700">
          ...
          <div className="relative">
            ...
            >
              ...
              <ChevronDown size={14} className={`transition-transform duration-200 ${isCategoriesMenuOpen ? "rotate-180" : ""}`} />
            </button>
                ...
                {CATEGORIES.map((cat) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-xs font-semibold hover:bg-gray-100 hover:text-mm-red transition-colors flex justify-between items-center ${
                      selectedCategory === cat ? "text-mm-red bg-red-50/50" : "text-gray-700"
                    }`}
                  >
                    <span>{cat === "Todos" ? "Todas las categorías" : cat}</span>
                    {selectedCategory === cat && <span className="h-1.5 w-1.5 rounded-full bg-mm-red" />}
                  </button>
                ))}
              </div>
            )}
